...
/* モノラルのWAVファイルを読み取り、MONO_PCM構造体に格納 */
void mono_wave_read(MONO_PCM *pcm, char *file_name) {
    ...
    pcm->s = (double *)malloc(pcm->length * sizeof(double)); /* メモリの確保 */

    for (n = 0; n < pcm->length; n++) {
        fread(&data, fmt_bits_per_sample / 8, 1, fp); /* 音データの読み取り */
        /* 音データを-1以上1未満の範囲に正規化する */
        pcm->s[n] = (double)data / short_max;
    }

    fclose(fp);
}

/* MONO_PCM構造体からモノラルのWAVファイル作成 */
void mono_wave_write(MONO_PCM *pcm, char *file_name) {
    ...
    for (n = 0; n < pcm->length; n++) {
        ...
        fwrite(&data, fmt_bits_per_sample / 8, 1, fp); /* 音データの書き出し */
    }
